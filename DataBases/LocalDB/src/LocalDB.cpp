...
int UserDB::selectDeviceId() 
  {
    ...
    std::vector<s_record> result;
    int device_id = exec(query.data(),[&result](const s_record& r,void* context)
      {
        result.emplace_back(r);
        return 0;
      },this);
...
int UserDB::selectLogin()
  {
    ...
    std::vector<s_record> result;
    int login = exec(query.data(),[&result](const s_record& r,void* context)
      {
        result.emplace_back(r);
        return 0;
      },this);
...
bool UserDB::selectPassword(const User& user) 
  {
    ...
    std::vector<s_record> result;
    int password = exec(query.data(),[&result](const s_record& r,void* context)
      {
        result.emplace_back(r);
        return 0;
      },this);
...
int UserDB::selectFolder() 
  {
    ...
    std::vector<s_record> result;
    int folder = exec(query.data(),[&result](const s_record& r,void* context)
      {
        result.emplace_back(r);
        return 0;
      },this);
...
int UserDB::selectLastUpdate() 
  {
    ...
    std::vector<s_record> result;
    int update = exec(query.c_str(),[&result](const s_record& r,void* context)
      {
        result.emplace_back(r);
        return 0;
      },this);
...
void UserDB::saveLastUpdate(User& user) 
  {
    ...
    std::string lastTMPUpdate = std::to_string(1900 + local_time->tm_year) + "-" + std::to_string(1 + local_time->tm_mon) + "-"
...
void FileDB::addFile(FileMeta &file)
  {
    ...
    auto query = "INSERT INTO Files (fileName, fileExtention, fileSize, filePath,"
        ...
        + std::to_string(date) + "', '" + std::to_string(date) + "');";
...
std::optional<FileMeta> FileDB::getOneFile(const int id)
  {
    ...
    "FROM Files where fileid="+std::to_string(id);
    std::vector<s_record> result;
    exec(query.data(),[&result](const s_record& r,void* context)
      {
        result.emplace_back(r);
        return 0;
      },this);
...
void FileMeta::fillMeFromRecord(const s_record& r)
  {
    *this={};
    for(auto& col:r)
       {
         if("fileName"      ==col.col_name){fileName      =         col.col_text; continue;}
         if("fileId"        ==col.col_name){fileId        =strtol(  col.col_text.c_str(),0,10); continue;}
         if("fileExtention" ==col.col_name){fileExtention =         col.col_text; continue;}
         if("fileSize"      ==col.col_name){fileSize      =strtol(  col.col_text.c_str(),0,10); continue;}
         if("filePath"      ==col.col_name){filePath      =         col.col_text; continue;}
         if("chunksCount"   ==col.col_name){chunksCount   =strtol(  col.col_text.c_str(),0,10); continue;}
         if("version"       ==col.col_name){version       =strtol(  col.col_text.c_str(),0,10); continue;}
         if("isDownload"    ==col.col_name){isDownload    =!!strtol(col.col_text.c_str(),0,10); continue;}
         if("updateDate"    ==col.col_name){updateDate    =         col.col_text; continue;}
         if("createDate"    ==col.col_name){createDate    =         col.col_text; continue;}
       }
  }
...
void FileDB::updateFile(FileMeta &file) 
  {
    ...
    auto query = "Update Files SET "
      ...
      "'" " WHERE fileId=" + std::to_string(file.fileId) + ";";
    file.updateDate = date;
